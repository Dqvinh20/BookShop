...
public class RestProductsRespository : IProductsRepository
{
    ...
    public async Task<IEnumerable<Product>> GetAllProductsAsync() => await _http.GetAsync<IEnumerable<Product>>($"{_controller}?select=*,categories(*)&order=name", _accessToken);
    public async Task<Product> GetProductByIdAsync(int id)
    {
        var data = await _http.GetAsync<IEnumerable<Product>>($"{_controller}?select=*,categories(*)&id=eq.{id}", _accessToken);
    ...
    public async Task<IEnumerable<Product>> UpsertProductAsync(List<Product> products)
    {
        ...
        return await _http.PostAsync<List<Product>, IEnumerable<Product>>($"{_controller}?select=*,categories(*)", products, _accessToken, headers);
    }
